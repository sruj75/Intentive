...
- `runtime-config.ts` reads the three public seams from `EXPO_PUBLIC_*` env (Control Plane base URL, Sentry DSN; the Neon Auth base URL is read inside `neon-client.ts`) plus `expo-constants` and `__DEV__`. Blank values keep a seam dormant.
...
- Apple and Google both report `not-configured` today (no credentials / rejected callback scheme per `neon-client.ts`); re-enabling them is a one-line change to `NEON_ENABLED_PROVIDERS` once the round-trip completes. The dev provider stays `__DEV__`-only and is no longer reachable from a button — it is exercised via injection.
