...
ADR 0013's original screen meaning is retained. Amended 2026-07-25: the mounted
production gate now persists `POST /consent` through the Launch State boundary
and reconciles `GET /me` before advancing; it no longer asserts completion
locally.
...
- Split into two gates: a data-privacy acceptance _and_ a memory explainer. Rejected: the memory explanation belongs in-chat (ADR 0006), and a second pre-chat screen with no acceptance is exactly the over-modeling ADR 0019 avoids.
...
- **Durable `POST /consent` and cross-client suppression are owned by the Control
  Plane.** No decline path exists (`GateStatus` has no `declined`); "Agree &
  Continue" is the only first-party action.
