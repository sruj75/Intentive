...
| `DESKTOP_CONTROL_PLANE_URL`        | repository variable | Production Control Plane HTTPS URL embedded in the app bundle                   |
| `DESKTOP_HOSTED_AUTH_URL`          | repository variable | Production hosted-auth HTTPS entry point embedded in the app bundle             |
| `DESKTOP_AUTH_TOKEN_EXCHANGE_URL`  | repository variable | Optional HTTPS code-exchange endpoint for hosted auth                           |
...
2. Review the uploaded exact-SHA evidence. Start `desktop-release.yml` with the accepted SHA, candidate workflow run ID, and version. The job pauses at the protected `desktop-release-approval` environment; after approval it validates the evidence and reserves `desktop-vX.Y.Z` (or `desktop-vX.Y.Z+BUILD`) as the release identity without publishing the tag yet.
3. The protected workflow validates and embeds the configured HTTPS Control Plane and hosted-auth endpoints, builds the release app, signs Sparkle/Sentry inside-out, signs the app with `Intentive-Release.entitlements`, creates an installable DMG with an `/Applications` link, signs/notarizes/staples it, and creates the Sparkle signature from that exact DMG.
4. `smoke-signed-desktop-artifact.sh` verifies identity, embedded non-local HTTPS service endpoints, Team ID, hardened runtime, Gatekeeper, arm64-only architecture, frameworks/assets/privacy metadata, DMG ticket and contents, appcast metadata, and the cryptographic Sparkle signature. It writes digest evidence.
5. Only after that audit succeeds does the workflow create and push the annotated tag, immediately followed by a **draft** GitHub Release. A rerun may reuse the tag only when it already resolves to the accepted candidate SHA. Draft status is load-bearing: artifacts are not exposed through Sparkle before dedicated-Mac acceptance.
