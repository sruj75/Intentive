...
- Passive audio is on by default and activates once the user grants microphone permission; it never fills the Floating Bar composer, retains no raw audio, and synchronizes only compact hard-secret-filtered summaries. It remains fully user-controllable and still requires mic permission, still pauses in Private Mode, and still honors excluded apps.
- The microphone uses a continuous CoreAudio source only while desired policy, permission, and Private Mode allow it. The system process tap starts only for a detected meeting (with off-hysteresis) unless the user explicitly chooses another system-audio mode; failure of that optional tap does not stop microphone sensing.
