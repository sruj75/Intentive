...
- **Helper bundle layout.** The ScreenPipe binary is relocated from a flat `resources/screenpipe` into a child app bundle:
  `Intentive Capture.app/Contents/Info.plist` (`CFBundleDisplayName = Intentive`, `CFBundleName = Intentive`, `CFBundleIdentifier = com.heyintentive.capture`, `CFBundleExecutable = screenpipe`, `CFBundleIconFile = icon.icns`), `Intentive Capture.app/Contents/Resources/icon.icns`, and `Intentive Capture.app/Contents/MacOS/screenpipe`. The helper icon is copied from the main Desktop Client icon so macOS Privacy Settings shows the Intentive logo rather than a blank/default app icon. The capture supervisor's resolved spawn path changes accordingly (today `resources/screenpipe` at `lib.rs`; see the plan doc). The child-process / HTTP boundary of ADR-0002 is unchanged — only the on-disk shape and the TCC-visible identity change. Ollama remains a flat signed resource (it is never a TCC principal).
...
- **Bundle config.** `productName` becomes `Intentive` (capitalized — lowercase `intentive` is a release blocker per the decision statement), `bundle.targets` is `["app", "dmg"]` so Tauri emits updater artifacts alongside the DMG, and `createUpdaterArtifacts: true` is set for the updater (ADR-0024). The signing identity is supplied via CI env (Developer ID Application: Srujan Gowda, Team `24D6NXS6H7`).
...
- Release smoke must verify the menu bar item, macOS Privacy Settings row label **Intentive** with the Intentive logo, Login Items when enabled, ScreenPipe health on `127.0.0.1:44380`, frame/audio writes, stop cleanup, quit cleanup, and absence of debug, blank/default-icon, or ScreenPipe-facing identity.
