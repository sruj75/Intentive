...
- Passive microphone and system-audio sensing is separate from conversation input and deferred until the local Omi-derived pipeline is restored in Slice 8. The legacy RunAnywhere path does not warm up or start.
...
- Sparkle, telemetry, passive audio, Screen Memory archive restoration, and wider settings renovation retain their own slices; this ADR fixes their boundaries without implementing them early.
