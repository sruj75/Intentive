...
- **Monitor:** #32 ships a poll-only monitor on a ~5s interval. It emits a "readiness lost" event on revocation; it does **not** stop capture itself. A **10s wake-grace** suppresses events after system sleep/wake to avoid false positives. The ~100ms eager detection path from capture-stream errors is deferred to #43 alongside the reliability harness work; until then, a ScreenPipe crash during capture is classified as `Error` first and the monitor poll decides whether permission is gone.
- **Subscriber:** the capture coordinator owns the FSM transition. Readiness-false ⟹ `SetupRequired` (ADR-0020), at first-run and mid-session alike. Mirroring ScreenPipe's bidirectional `report_state` (which can correct state in either direction), the poll is the single permission authority and can pull `Error → SetupRequired` when it confirms a grant is gone; eager `SCStream`-error detection of effective-until-quit revocation that the probe never reflects remains #43.
