...
- **Subscriber:** the capture coordinator owns the FSM transition. Readiness-false ⟹ `SetupRequired` (ADR-0020), at first-run and mid-session alike. Mirroring ScreenPipe's bidirectional `report_state` (which can correct state in either direction), the poll is also the backstop that pulls `Error → SetupRequired` when a permission-caused crash was misclassified as `Error` because the crash-time probe lagged the revocation; eager `SCStream`-error detection of effective-until-quit revocation that the probe never reflects remains #43.
