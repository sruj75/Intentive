...
- **Monitor:** a ~5s poll plus ~100ms eager detection from capture-stream errors. It emits a "readiness lost" event on revocation; it does **not** stop capture itself. A **10s wake-grace** suppresses events after system sleep/wake to avoid false positives.
