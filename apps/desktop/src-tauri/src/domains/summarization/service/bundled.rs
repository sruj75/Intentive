...
use crate::providers::port::resolve_port;
#[cfg(test)]
use crate::providers::port::resolve_port_with;
...
/// HTTP probe timeout for `is_running` and `/api/tags` checks. In practice
/// bundled Ollama can take >500ms to answer while runners warm up, so use a
/// slightly wider timeout to avoid false negatives during onboarding.
