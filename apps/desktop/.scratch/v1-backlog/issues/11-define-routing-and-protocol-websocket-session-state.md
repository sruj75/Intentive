...
This issue establishes the local app state and interfaces that snapshot delivery (#8), smoke (#9), and verification (#10) consume — without exposing `ws_url` or tokens in the Settings UI.
...
- [ ] Provide a stub or fixture path so #8–#10 can develop against resolved Routing before production Control Plane is wired.
- [ ] Document where this state lives and how #9 obtains it for smoke.
