...
struct OnboardingFloatingBarShortcutStepView: View {
                    ...
                    Text(isRecordingCustomShortcut ? "Listening..." : "Save")
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 10, style: .continuous)
                                .fill(IntentiveColors.backgroundPrimary)
                        )
                }
                .buttonStyle(.plain)
    ...
    private func keyCap(_ label: String) -> some View {
        RoundedRectangle(cornerRadius: 10, style: .continuous)
            .fill(shortcutDetected ? Color.white : IntentiveColors.backgroundTertiary)
            .frame(minWidth: 48, minHeight: 48)
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(
                        shortcutDetected ? Color.white : IntentiveColors.textTertiary.opacity(0.3),
                        lineWidth: 2
                    )
            )
            .overlay {
                Text(label)
                    ...
                    .foregroundColor(shortcutDetected ? .black : IntentiveColors.textPrimary)
    ...
    private func smallKeyCap(_ label: String, active: Bool) -> some View {
        RoundedRectangle(cornerRadius: 8, style: .continuous)
            .fill(active ? Color.white : IntentiveColors.backgroundTertiary)
            .frame(minWidth: 36, minHeight: 32)
            .overlay {
                Text(label)
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundColor(active ? .black : IntentiveColors.textPrimary)
    ...
    private func captureCustomShortcut(from event: NSEvent) -> Bool {
        if event.type == .flagsChanged {
            captureError = "The Floating Bar shortcut needs a non-modifier key."
            return true
        }
