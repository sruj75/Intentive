...
struct RewindTimelineView: View {
    ...
    var body: some View {
        ...
        .background(
            IntentiveColors.backgroundSecondary.opacity(0.95)
    ...
    var body: some View {
        Button(action: onTap) {
            VStack(spacing: 4) {
                ...
                Circle()
                    .fill(isActive ? IntentiveColors.purplePrimary : Color.clear)
            ...
            .background(
                RoundedRectangle(cornerRadius: 6)
                    .fill(isHovered ? IntentiveColors.backgroundTertiary : Color.clear)
            )
        }
        .buttonStyle(.plain)
        .onHover { hovering in
...
struct ScreenshotPreviewView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .padding(.horizontal, 16)
            .padding(.vertical, 12)
            .background(IntentiveColors.backgroundSecondary.opacity(0.95))
                ...
                ZStack {
                    if let image = fullImage {
                        ...
                        Image(systemName: "photo")
                            .scaledFont(size: 48)
                            .foregroundColor(IntentiveColors.textQuaternary)
                    }
    ...
    private func highlightedText(_ text: String, query: String) -> AttributedString {
        ...
        result.foregroundColor = IntentiveColors.textSecondary
        ...
        while let range = lowercasedText.range(of: lowercasedQuery, range: searchStart..<lowercasedText.endIndex) {
            if let attrRange = Range(range, in: result) {
                result[attrRange].foregroundColor = IntentiveColors.purplePrimary
                result[attrRange].backgroundColor = IntentiveColors.purplePrimary.opacity(0.2)
    ...
    var body: some View {
        ...
        ZStack {
            ForEach(Array(matchingBlocks.enumerated()), id: \.offset) { _, block in
                ...
                Rectangle()
                    .stroke(IntentiveColors.purplePrimary, lineWidth: 2)
                    .background(IntentiveColors.purplePrimary.opacity(0.2))
                    .frame(
