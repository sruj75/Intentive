...
struct ShortcutsSettingsSection: View {
          ...
          RoundedRectangle(cornerRadius: 10)
            .fill(
              (isSelected || recordingTarget == target)
                ? IntentiveColors.purplePrimary.opacity(0.3)
                : IntentiveColors.backgroundTertiary.opacity(0.5))
        )
        .overlay(
          RoundedRectangle(cornerRadius: 10)
            .stroke(
              isSelected || recordingTarget == target ? IntentiveColors.purplePrimary : Color.clear,
              lineWidth: 1.5)
        )
    }
    .buttonStyle(.plain)
  ...
  private func disableShortcutButton(isDisabled: Bool, action: @escaping () -> Void) -> some View {
    Button(action: action) {
      Text("Disable")
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 10)
            .stroke(isDisabled ? IntentiveColors.purplePrimary : Color.clear, lineWidth: 1.5)
        )
    }
    .buttonStyle(.plain)
  }
  ...
  private func shortcutSelectionLabel(tokens: [String], isSelected: Bool) -> some View {
    ...
    .overlay(
      RoundedRectangle(cornerRadius: 10)
        .stroke(isSelected ? IntentiveColors.purplePrimary : Color.clear, lineWidth: 1.5)
    )
  }
  ...
  private func shortcutRecorderCard(
    ...
    .background(
      RoundedRectangle(cornerRadius: 12)
        .fill(IntentiveColors.backgroundSecondary.opacity(0.85))
    )
  }
