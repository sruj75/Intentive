...
Messages enter the UI through the Protocol WebSocket client boundary established in #27 (not a local durable message store). After reconnect, render the server-provided timeline in order; append outbound and inbound events as the live session progresses.
...
- [ ] User sends and assistant replies flow through the #27 Protocol client boundary; the UI does not invent or persist a parallel transcript.
- [ ] After app restart, thread continuity comes from the reconnect snapshot, not from on-device message storage.
- [ ] Tests cover snapshot hydration ordering, live append during session, delivery-status updates, malformed snapshot rejection, and empty-history first open (bootstrap opening owned by #27/#39).
