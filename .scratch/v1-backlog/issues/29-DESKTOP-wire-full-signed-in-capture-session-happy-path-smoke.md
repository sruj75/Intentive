...
- [ ] The **Context Snapshot** is emitted on the Protocol WebSocket to a controlled test gateway (#28).
...
- [ ] The smoke path verifies Settings/sign-in copy does not use legacy OpenClaw Agent, Agent Interface, or manual endpoint language (#25).
...
This runtime happy-path smoke should meet the final packaged-app smoke at the end of v1. Keep the existing signed-in Capture Session path, and reference #49 for the final installed-app release bar. The runtime smoke is not complete launch evidence unless the final notarized `/Applications/Intentive.app` flow also proves:
...
The existing issue should stay focused on the runtime happy path; the packaged release smoke belongs in #49 and should be a blocker for calling #37 launch-complete.

### 02 @sruj75 — 2026-05-21T12:34:56Z

Follow-up links from the packaging issue pass:

- Final packaged-app release smoke now lives in #49 and blocks calling the runtime happy path launch-complete.
- Release packaging lives in #47.
- Product-owned macOS Privacy Settings identity lives in #48.
- Capture Permission Setup lives in #26.

Keep this issue focused on runtime signed-in Capture Session behavior; #49 is the final installed-app release bar.
