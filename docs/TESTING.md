...
- `.github/workflows/security-audit.yml` runs GitHub dependency review on pull requests only when pnpm dependency inputs change; its weekly/manual path runs the full `pnpm audit --audit-level high`.
