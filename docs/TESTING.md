...
- `.github/workflows/control-plane-ci.yml` runs Control Plane typecheck and the full test suite (including the opt-in Neon repo integration test when repository Actions values are set) on pull requests that touch `services/control-plane/` or its shared-package dependencies.
- `.github/workflows/neon-preview-branches.yml` creates one Neon branch per Control Plane pull request, validates migrations against it, and deletes it when the PR closes.
