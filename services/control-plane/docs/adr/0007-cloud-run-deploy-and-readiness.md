...
The smoke check targets a **deep readiness probe**, not the shallow liveness probe. `GET /health` stays a dumb liveness check (process up; Cloud Run uses it to restart crashed instances). A new `GET /ready` performs a _real_ dependency check — a `SELECT 1` against Neon and a fetch of the Neon Auth JWKS — and returns 200 only when both succeed. The deploy smoke check calls `/ready`, so "green" means "this revision can actually reach the things it needs," catching a deploy that boots but cannot reach the database or the auth keys.

## Considered Options

- **No-traffic revision promotion gated by `/ready` (chosen).** Gives "new code proves itself before users see it" (the row-7 failure: a deploy ships broken code) without the cost of a second Neon database, a second Cloud Run service, and a second secret set. Neon's HTTP driver already removes the connection-exhaustion risk that a separate environment would otherwise help isolate.
...
- **Smoke-test the authenticated endpoints (`/me`, `/agent`, `/devices/register`) against prod.** Rejected: each needs a real Neon Auth user JWT, `/agent` would fire a live Session Start, and `/devices/register` would write a junk row into the production database on every deploy. `/ready` proves the two things that actually break (Neon + JWKS); route wiring is already covered by the test suite against ephemeral Neon branches (ADR-0003).

## Consequences

- A new `GET /ready` handler lives in the identity `ui` layer beside `/health`; it must fail fast (short timeout) and must not become an unauthenticated "ping my database" amplifier.
...
- Structured logging emits **domain events** (JWT verification outcomes, gate transitions, Session Start calls, device registrations, push fan-out results) through `bootstrapObservability` in the `packages/providers` telemetry boundary with redaction (no tokens, push tokens, device fingerprints, or shared secrets). It does **not** duplicate the per-request logs Cloud Run already emits at the edge.
- Sentry is the Control Plane error-capture sink: `SENTRY_DSN` is vaulted, while `SENTRY_ENVIRONMENT`, `SENTRY_RELEASE`, and `SENTRY_MODE` are plain environment variables (`errors-only` by default). Langfuse is intentionally absent because this service has no LLM trace surface.
...
- The first production bootstrap uses `us-west1` because this GCP project hit a Cloud Run project-initialization quota failure in `us-central1`; moving regions later is a deploy/IAM/secrets exercise, not an application architecture change.
