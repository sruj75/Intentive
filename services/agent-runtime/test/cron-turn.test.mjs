...
test("cron turn pushes the committed recurring lifecycle onto the reschedule hook", async () => {
  const rescheduled = [];
  const cancelled = [];
  const sql = { transaction: async (queries) => queries };
  const handler = createCronTurnHandler({
    cronJobs: {
      deleteQuery: (id) => Promise.resolve([{ id, op: "delete" }]),
      rescheduleQuery: (id, nextFireAt, attemptCount) =>
    ...
    cronRuns: { recordQuery: (record) => Promise.resolve([{ id: "run_1", record }]) },
    floorResolver: { resolve: async () => floor("floor_v1") },
    onRescheduleCron: (job, nextFireAt) => rescheduled.push([job.id, nextFireAt.toISOString()]),
    ...
    turn: createTurn({
      ...
      runtimeTurns: { recordQuery: (record) => Promise.resolve([{ record }]) },
      fallbackModel: "fallback-model",
    }),
  });

  // A successful recurring (`every 5m`) fire reschedules forward from firedAt.
  await handler(job({ scheduleKind: "every", scheduleExpr: "5m", tz: "UTC" }), {
    firedAt: new Date("2026-06-16T00:00:00.000Z"),
  });

  assert.equal(cancelled.length, 0);
  assert.equal(rescheduled.length, 1);
  assert.equal(rescheduled[0][0], "job_1");
  // croner computes the next 5m boundary from 00:00 UTC → 00:05.
  assert.equal(rescheduled[0][1], "2026-06-16T00:05:00.000Z");
});

test("cron turn pushes a one-shot delete onto the cancel hook", async () => {
  const cancelled = [];
  const rescheduled = [];
  const sql = { transaction: async (queries) => queries };
  const handler = createCronTurnHandler({
    cronJobs: {
      deleteQuery: (id) => Promise.resolve([{ id }]),
      rescheduleQuery: () => Promise.resolve([]),
    },
    cronRuns: { recordQuery: (record) => Promise.resolve([{ record }]) },
    floorResolver: { resolve: async () => floor("floor_v1") },
    onRescheduleCron: (job, nextFireAt) => rescheduled.push([job.id, nextFireAt]),
    onCancelCron: (id) => cancelled.push(id),
    turn: createTurn({
      sql,
      adapter: {
  ...
  await handler(job({ scheduleKind: "at" }), {
    firedAt: new Date("2026-06-16T00:00:00.000Z"),
  });

  assert.deepEqual(cancelled, ["job_1"]);
