...
- **Phase 0 contracts resolved** ([Issue #10]) — all pre-implementation contracts are now
  ...
  - **Persistence Adapter** — thin repo-owned wrapper around LangGraph's Postgres checkpoint
    store; shell domains never import LangGraph checkpoint types directly (ADR-0036 context).
  ...
  - **Session Snapshot shape** — `hello_ok.session_snapshot` is now a typed
    `{ messages: SessionMessage[], before_cursor: string | null }` projection; see ADR-0037.
  ...
  - **Bundle version pinning** — resolved once per WebSocket connection at `hello_ok` from
    ...
    recorded on each `runtime_turns` row for the eval loop (ADR-0006 amendment).
...
- **[ADR-0036]** `docs/adr/0036-agent-runtime-vfs-write-policy-and-deferred-self-personalization.md`
  — formalises the VFS write policy and defers agent-driven behavioral self-personalization.

- **[ADR-0037]** `docs/adr/0037-agent-runtime-session-snapshot-as-separate-projection.md`
...
- **`packages/protocol` — `hello_ok.session_snapshot`** replaced `z.unknown()` with
  ...
  `SessionSnapshot`). Exportable from `@intentive/protocol` (ADR-0037).
...
- **`docs/adr/0006`** amended inline — bundle version pin boundary clarified to
...
- **`docs/adr/README.md`** — ADR-0036 and ADR-0037 indexed; ADR-0006 marked as refined
  and amended; next ADR number bumped to `0038`.
