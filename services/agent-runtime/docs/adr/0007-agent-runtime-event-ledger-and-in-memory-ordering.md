...
Inbound flow: parse at the boundary → `INSERT INTO runtime_events … ON CONFLICT DO NOTHING` → if the row was newly inserted, enqueue it; if it conflicted, it is a duplicate and is silently dropped. The database write _is_ the duplicate check, in one step. Idempotency is enforced by a unique constraint `(user_id, kind, dedup_key)`, adopting the Control Plane's production idempotency pattern (unique constraint + `ON CONFLICT DO NOTHING`, as in `devices`/`users`). `dedup_key` is the client-supplied id for client wire events (`message_id`, `event_id`) and a runtime-minted id for events with no natural id (`session_end_marker`, and future system events) — which are therefore never deduped, matching OpenClaw's "no id → no dedupe."
