...
**4. Secrets come from Secret Manager, fetched at boot.** A short entrypoint in the image uses the VM's **dedicated** service account (scoped to `secretmanager.secretAccessor`, not the default Compute SA) to pull secrets from Secret Manager, then `exec`s the process. Secrets never sit in instance metadata. This is the same secret store the Control Plane uses — one secrets story across the system.
