...
**Heartbeat**:
The Companion's **always-on proactivity engine**: the periodic trigger that wakes a **Monitoring Turn** in the main session — _not_ a separate brain or monitoring loop (this matches OpenClaw, where the heartbeat runs a periodic agent turn in the main session _as the_ proactivity mechanism). It fires on cadence **regardless of capture session or connection state** — it is the reason the agent is proactive at all, so it must reach the user whether they are at a laptop, at the gym, or driving. It is **not** gated on **Context Snapshots** flowing: a Heartbeat tick and an arriving Context Snapshot are the **two triggers of the one Monitoring Turn** (ADR-0015), but they are not equal in role — the Heartbeat is the engine, the snapshot merely _enriches_ a turn when present. On a tick the main agent reads the immutable `HEARTBEAT.md` procedure (how to evaluate) + its `MEMORY.md` watch-list (what to watch) + whatever it knows (temporal grounding, durable memory, mobile signals, and the sensory buffer _if_ snapshots exist), then judges for itself whether to intervene. It fires on a **flat ~60-min floor** (the active regime is already driven by perception at ~10 min, ADR-0023, so the floor only bites when perception is absent), via a **computed poll-loop that stores no heartbeat state** — the scheduler mirrors the Cron poll loop (ADR-0024) but computes due-ness from the existing `runtime_events`/`runtime_turns` timestamps; there is **no heartbeat table, no `next_fire_at`, no cadence/liveness row** (ADR-0027). It **never backs off**: a quiet user is the moment to intervene more, not less, and "care less about a dormant user" would be the shell making a saliency judgment (forbidden, ADR-0014) — the shell fires the flat floor forever, the agent decides re-engage-vs-leave-alone. **Quiet-hours is agent judgment, not a stored or shell-predicted window:** the agent gathers the user's schedule during onboarding (`BOOTSTRAP.md`) into `USER.md` (eager-injected every turn) and simply does not **Post-Message-Back** while they sleep — a fixed window would break night owls/night-shift workers, and deciding "good time to reach them?" is the agent's call (ADR-0014). Deferred cost levers, bound by _never reduce reach, only reduce redundant reasoning_: **agent-controlled cadence** (the agent lengthens its own next wake by judgment) and a cheap shell **structural quiet-gate** (skips the overnight LLM call), the latter sourced via the cron-card frontmatter pattern (ADR-0026) — neither built in v1. Distinct from **Cron** because it is periodic/cadence-driven rather than absolute-time. Like Cron, a tick may or may not produce a **Post-Message-Back**.
...
- One process-wide **scheduler** holds all users' heartbeat cadence + cron due-times. Heartbeat wakes the main per-user lane; issue #39 Cron fires through the scheduler's silent ephemeral path until Post-Message-Back/main-thread delivery lands (ADR-0018; ADR-0017 amendment).
...
- "Heartbeat quiet-hours" as a stored/derived shell window, and "dormancy backoff" as a cadence policy — resolved 2026-06-16 (#40/#41, ADR-0027). v1 stores **no** heartbeat state at all: cadence is a **flat ~60-min floor** from a computed poll-loop (no table), and quiet-hours is **agent judgment** off the onboarding-gathered `USER.md`, never a fixed/global/predicted window (it would break night owls and night-shift workers). The shell **never backs off** a quiet user — dormancy is the highest-signal moment to intervene, and a shell deciding to "care less" is a saliency call ADR-0014 forbids. The cheap shell **structural quiet-gate** and **agent-controlled cadence** are deferred levers, bound by _never reduce reach, only reduce redundant reasoning_. #40 ships **with** #41 so the main-session Monitoring Turn has a real Post-Message-Back egress (and Cron flips main-session at the same time) — heartbeat fired with a stubbed egress would poison the checkpoint with phantom nudges.
...
- **Lazy hydration (wakes on demand).** When a main-thread trigger fires for a user — a `user_message`, a Context Snapshot, or a Heartbeat wake — the runtime builds that user's brain in memory _then_, by hydrating the checkpoint, and runs the turn on the Per-User Channel (ADR-0016). Issue #39 Cron uses a silent ephemeral thread and does not hydrate the main checkpoint.
...
- **One process-wide scheduler drives offline triggers.** A single shell-owned scheduler holds every user's cron due-times (from Neon) and heartbeat cadence. Heartbeat enqueues onto the Per-User Channel, hydrating the lane. Cron fires through the scheduler's ephemeral path in issue #39. This is what keeps offline proactivity available without a live socket; user-facing delivery returns with Post-Message-Back.
