...
**Cron**:
The Agent Runtime's scheduled-trigger primitive. Lets the agent decide on its own time ("ping the user at 9am tomorrow about the deadline"). Adapts OpenClaw's three schedule kinds via the **`croner`** library: **`at`** (one-shot), **`every`** (fixed interval), and **`cron`** (5/6-field recurring expression). Each job is a **cron card** — a markdown file the agent authors with DeepAgents' built-in filesystem tools (`write_file`/`edit_file`/`ls`) under a reserved `/crons/` route on the Neon `StoreBackend`, carrying the OpenClaw card fields (`name`, `schedule`, `prompt`/intent, `session`, `tz`) plus a shell-computed **next-fire time**. There are **no bespoke cron CRUD tools**: scheduling is file I/O, exactly like memory (ADR-0026). The card is a file to the agent and a Neon row underneath — which dissolves the file-vs-DB question (OpenClaw's prod store is a DB; the tutorial's on-disk `CRON.md` is the single-user form; ours is both at once because the VFS _is_ Postgres). A shell-side **poll loop** (not an in-memory timer wheel) finds due jobs in Neon (`next_fire_at <= now()`), so restart-survival and missed-fire handling fall out of one query (ADR-0024). Schedules are validated against a **5-minute minimum interval** (anti-spam / alarm-fatigue floor). A failed fire is retried on **transient errors only** (OpenClaw's `maxAttempts`/`backoffMs`/`retryOn` = rate-limit, overloaded, network, server-error) — these fail _before_ a turn delivers, so a retry re-attempts an un-delivered run, never re-nags; the agent _choosing to stay silent_ is a success, not a retryable failure. In v1 a Cron fire is a **scheduled main-session wake** — it runs against the user's one checkpoint on the **Per-User Channel** (like a timer-triggered Monitoring Turn), so a reminder is grounded in the full relationship context, not an isolated blind turn. Silent-by-default with **Post-Message-Back** egress (ADR-0013); the agent decides whether the trigger is worth interrupting for. Unlike a Monitoring Turn, a Cron fire also persists a durable **job/run record** ("cron schedules; tasks record what ran"). OpenClaw's **isolated**/custom cron sessions are **deferred** to the first background-chore use case (see ADR-0017). Distinct from **Heartbeat** (periodic/liveness) and the **Monitoring Turn** it shares the main-session lane with. `every` and Heartbeat coexist (as in OpenClaw): an `every` Cron is a specific, user-scoped recurring job that records runs; Heartbeat is the always-on engine that records nothing.
...
- **Proactive user-facing output happens only via the Post-Message-Back tool.** Self-scheduling needs **no bespoke tool**: the agent writes a **cron card** with the built-in filesystem tools (`write_file`/`edit_file`/`ls`) into a reserved `/crons/` route on the Neon `StoreBackend` — the same DeepAgents-native file-I/O pattern as memory. The shell-side write-route validates the schedule (croner + 5-min floor) and computes `next_fire_at`. Post-Message-Back stays an explicit tool because egress has external blast radius and must be auditable; scheduling does not.
...
- DeepAgents' **built-in** VFS tools (`read`/`write`/`edit`/`ls`/`grep`/`glob`) over the native `StoreBackend` on Neon. One file surface, two reserved routes: `USER.md`/`/memories/` for memory, and `/crons/` for **cron cards** — the agent _schedules itself_ by writing a card, not by calling a custom tool. Native file I/O, no bespoke cron CRUD verbs (ADR-0026).
