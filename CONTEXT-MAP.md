...
- **Wire/HTTP-contract and cross-cutting terms** (e.g. **Protocol**, **Perception Event**, **Session End Marker**, **Internal API**) → **`packages/CONTEXT.md`** (the Shared context), since they're imported by multiple deployables.
...
> **Domain expert:** "Yes, but not as a separate chat product. Desktop has a compact floating bar that joins the one Companion conversation. Its Screen Memory perception arrives separately as `perception_event` records."
...
- "Tauri pushes snapshots via HTTPS webhook to a per-user GCP VM" appeared in an older Desktop context. Rejected: the Agent Runtime is **Multi-Tenant** (no per-user VM) and the client↔runtime path is the same WebSocket **Protocol** for every client. The Desktop Client now sends `perception_event` records on that WebSocket.
...
- "Defer notification permission until Follow-Up exists" appeared in `apps/mobile/CONTEXT.md`. Rejected: with Post-Message-Back in v1, notifications are a v1 capability. Superseded 2026-07-02 (ADR 0018/0019): the OS permission **prompt** fires in the Onboarding funnel's Grant Permissions step; Expo Push Token registration still happens around first chat entry without re-prompting once permission is decided.
...
- "Desktop Client is capture-only" is superseded by the SwiftPM desktop renovation: Desktop now includes Screen Memory, floating-bar chat, push-to-talk voice, and an Effect Runner, while still keeping Screen Memory perception outside Conversation History.
